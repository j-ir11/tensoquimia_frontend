...
import { 
  ...
  Database, AlertTriangle, RefreshCw, Loader2, ChevronDown, X, Check
} from 'lucide-react';
...
const FormulaEditor = () => {
  ...
  const [originalData, setOriginalData] = useState({ recipe: [], nombreProceso: 'NINGUNO', localFP: "" });
    ...
    const loadPrev = async () => {
      if (target) {
        ...
        if (data && data.version) {
          ...
          const loadedRecipe = data.ingredientes ? data.ingredientes.map(ing => ({
          ...
          setOriginalData({ recipe: loadedRecipe, nombreProceso: nProc, localFP: fProc });

          const predefinidos = { 'CALOR': 0.5, 'FRIO': 0.3, 'NINGUNO': 0 };
          const esPredefinido = predefinidos[nProc.toUpperCase()] !== undefined && 
                               Math.abs(predefinidos[nProc.toUpperCase()] - parseFloat(fProc)) < 0.0001;
        ...
        <div className="bg-white border border-slate-200 rounded shadow-lg overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y">
              {productos.filter(p => p.tipo_producto === 'PI' && p.descripcion_producto.toLowerCase().includes(query.toLowerCase())).map(p => (
    ...
    <div className="space-y-6 animate-fade">
          ...
          <div className="bg-white rounded-lg shadow-2xl border-t-4 border-indigo-500 w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              <div className="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4">
                <RefreshCw className="text-indigo-600 animate-spin-slow" size={32} />
              </div>
              ...
              <p className="text-xs font-bold text-slate-500 mt-2 leading-relaxed">
                Esta acción reemplazará la versión actual en la base de datos de forma permanente.
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-4 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
           <div className="p-4 bg-[#0f172a] text-white font-black text-[10px] uppercase flex justify-between items-center tracking-widest">Almacén de Insumos <Database size={14}/></div>
           ...
           <table className="w-full text-left border-collapse">
                   ...
                   <tr key={r.id_componente} className="hover:bg-slate-50/80 transition-all">
                     <td className="p-5">
                       <div className="flex items-center gap-4">
                        <button onClick={() => setRecipe(recipe.filter(x => x.id_componente !== r.id_componente))} className="text-slate-300 hover:text-red-500 transition-colors p-1 hover:bg-red-50 rounded"><Trash2 size={16} /></button>
                        <div>
                     ...
                     <td className="p-5 text-right font-mono text-slate-400 text-xs font-bold">${costoUnit.toFixed(3)}</td>
                     <td className="p-5">
                       <div className="bg-[#0f172a] rounded-lg p-1.5 flex items-center border border-slate-800 shadow-inner group">
                          <input 
                            ...
                            onBlur={(e) => {
                              ...
                              const formatted = (isNaN(num) || num < 0) ? "" : num.toFixed(3);
                     ...
                     <td className="p-5 text-right font-black text-slate-900 font-mono text-base bg-emerald-50/20 underline">${aporte.toFixed(3)}</td>
                   </tr>
                 );
               })}
             </tbody>
             <tfoot className="bg-slate-900 text-white">
                <tr className="font-black uppercase text-[10px] tracking-widest">
                    ...
                    <td className={`p-6 text-center font-mono text-base ${esValido ? 'text-emerald-400' : 'text-amber-400'}`}>{totalPorcentaje.toFixed(3)}%</td>
                    <td className="p-6 text-right font-mono text-xl text-emerald-400 italic">${costoFinalSimulado.toFixed(3)}</td>
                </tr>
             </tfoot>
           </table>
        </div>
